...
# --- BILD-FUNKTIONEN (Base64) ---
def compress_image(uploaded_file, max_size=800, quality=85):
    """Komprimiert ein Bild für Google Sheets (max ~50KB pro Bild)"""
    try:
        ...
        # Konvertiere zu RGB (JPEG braucht RGB)
        ...
        # Speichere als JPEG mit hoher Qualität
        buffer = BytesIO()
        img.save(buffer, format='JPEG', quality=quality, optimize=True)
        buffer.seek(0)
        
        # Prüfe Grösse - wenn zu gross, reduziere weiter
        b64 = base64.b64encode(buffer.getvalue()).decode('utf-8')
        
        # Google Sheets Zellen-Limit ist ~50KB
        while len(b64) > 45000 and quality > 50:
            quality -= 10
            buffer = BytesIO()
            img.save(buffer, format='JPEG', quality=quality, optimize=True)
            buffer.seek(0)
        ...
        # Wenn immer noch zu gross, verkleinere das Bild
        while len(b64) > 45000 and max_size > 400:
            max_size -= 100
            ratio = max_size / max(img.size)
            new_size = tuple(int(dim * ratio) for dim in img.size)
            img = img.resize(new_size, Image.Resampling.LANCZOS)
        ...
        raise Exception(f"Bildkompression fehlgeschlagen: {str(e)}")

def get_images_for_trade(trade_id, df):
    """Holt alle Bilder für einen Trade aus dem DataFrame"""
    if df.empty:
        return []
    ...
    images_json = trade_row.iloc[0].get('images', '[]')
    try:
...
def get_or_create_spreadsheet():
    ...
    try:
        ...
        # Erstelle Worksheets
        # Trades Sheet - mit images Spalte
        trades_ws = spreadsheet.sheet1
        trades_ws.update_title("Trades")
        trades_ws.update('A1:N1', [["id", "trade_id", "date", "time", "account", "asset", "direction", "pnl", "notes", "tags", "checklist", "reviewed", "created_at", "images"]])
...
def load_data():
    ...
    if not data:
        return pd.DataFrame(columns=["id", "trade_id", "date", "time", "account", "asset", "direction", "pnl", "notes", "tags", "checklist", "reviewed", "created_at", "images"])
    ...
    # Stelle sicher dass images Spalte existiert
...
def load_data_cached():
    """Alias für load_data - für Kompatibilität"""
    return load_data()
...
def save_entry(entry_data, mode="new"):
    ...
    # Images zu JSON konvertieren
    if isinstance(entry_data.get("images"), list):
        entry_data["images"] = json.dumps(entry_data["images"])
    elif "images" not in entry_data:
        entry_data["images"] = "[]"
    ...
    if mode == "edit":
        ...
        for i, row in enumerate(all_data[1:], start=2):
            if row[0] == entry_data["id"]:
                ...
                trades_ws.update(f'A{i}:N{i}', [new_row])
                return
    
    # Neue Zeile hinzufügen
    headers = ["id", "trade_id", "date", "time", "account", "asset", "direction", "pnl", "notes", "tags", "checklist", "reviewed", "created_at", "images"]
    new_row = [str(entry_data.get(h, "")) for h in headers]
...
with tab_input:
    ...
    with col_btn1:
        if st.button("💾 Trade Speichern", type="primary", use_container_width=True):
            ...
            # Bilder komprimieren und als Base64 speichern
            images_base64 = []
            upload_errors = []
            if i_files:
                for img_num, uploaded_file in enumerate(i_files, start=1):
                    try:
                        b64_image = compress_image(uploaded_file)
                        images_base64.append({
                            "name": f"{trade_id}_{img_num:02d}.jpg",
                            "data": b64_image
                        })
                    except Exception as e:
                        upload_errors.append(f"{uploaded_file.name}: {str(e)}")
            ...
            # Speichere Ergebnisse in Session State für Anzeige nach rerun
            if images_base64:
                st.session_state["uploaded_images"] = [img["name"] for img in images_base64]
            if upload_errors:
            ...
            # ALLE Form-Felder zurücksetzen für neuen Trade
            keys_to_reset = [
                "input_date", "input_time", "input_account", "input_asset",
                "input_direction", "input_pnl", "input_tags", "input_notes", 
                "input_files"
            ]
            ...
            for k in cl_data.keys():
                if f"new_{k}" in st.session_state:
                    del st.session_state[f"new_{k}"]
                ...
                with st.expander(expander_title, expanded=False):
                    ...
                    if st.session_state[edit_key]:
                        ...
                        with vc1:
                            ...
                            # Bilder aus Base64 laden
                            images_json = row.get("images", "[]")
                            try:
                                if isinstance(images_json, str) and images_json:
