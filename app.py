...
# Google Drive TradingJournal Hauptordner ID (nicht der images Unterordner!)
# Finde diese ID: Öffne den TradingJournal Ordner in Drive, die ID ist in der URL
PARENT_FOLDER_ID = "1Nxv1uwSu-FP-WCE-sTzqSaQTeMIGFWWg"
...
def get_or_create_images_folder():
    """Holt oder erstellt den images Ordner in Google Drive"""
    drive_service = get_drive_service()
    
    # Suche nach existierendem "images" Ordner im Parent
    query = f"name='images' and '{PARENT_FOLDER_ID}' in parents and mimeType='application/vnd.google-apps.folder' and trashed=false"
...
def upload_image_to_drive(uploaded_file, trade_id, image_number):
    ...
    try:
        ...
        # Metadata für die Datei - in den images Ordner
        file_metadata = {
            'name': filename,
        ...
        # supportsAllDrives für freigegebene Ordner
            ...
            if i_files:
                for img_num, uploaded_file in enumerate(i_files, start=1):
                    try:
                        ...
                        uploaded_images.append(result['name'])
                    except Exception as e:
