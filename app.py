...
def upload_image_to_drive(uploaded_file, trade_id):
    ...
    # Dateiname mit Trade-ID
    file_extension = uploaded_file.name.split('.')[-1]
    filename = f"{trade_id}_{uuid.uuid4().hex[:6]}.{file_extension}"
        ...
        if st.button("💾 Trade Speichern", type="primary", use_container_width=True):
            ...
            if i_files:
                with st.spinner("Bilder werden hochgeladen..."):
                    for uploaded_file in i_files:
                        try:
                            upload_image_to_drive(uploaded_file, trade_id)
                        except Exception as e:
