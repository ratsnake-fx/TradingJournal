...
def compress_image(uploaded_file, max_size=1024):
    """Komprimiert ein Bild lossless (PNG) und gibt Base64 zurück"""
    try:
        img = Image.open(uploaded_file)
        
        # Behalte Original-Format wenn möglich
        original_format = img.format or 'PNG'
        
        # Resize wenn zu gross (behält Qualität)
        if max(img.size) > max_size:
            ratio = max_size / max(img.size)
            new_size = tuple(int(dim * ratio) for dim in img.size)
            img = img.resize(new_size, Image.Resampling.LANCZOS)
        
        # Speichere als PNG (lossless)
        buffer = BytesIO()
        
        # Konvertiere RGBA zu RGB wenn nötig für kleinere Dateigrösse
        if img.mode == 'RGBA':
            # Behalte Transparenz
                ...
                for img_num, uploaded_file in enumerate(i_files, start=1):
                    try:
                        ...
                        images_base64.append({
                            "name": f"{trade_id}_{img_num:02d}.png",
                            "data": b64_image
                        })
                    except Exception as e:
